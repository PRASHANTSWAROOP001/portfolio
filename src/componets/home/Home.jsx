...
//by default nightMode is false that means its a dayMode
  ...
  return (
    <div className="w-full h-screen    dark:bg-gradient-to-r dark:from-slate-900 dark:to-slate-700 dark:text-white relative  ">
      ...
      <div className="fixed bottom-3 w-full flex  pb-4 justify-center">
        <IoSunnyOutline
          onClick={toggleIcon}
          className={
            !dayMode
              ? "hidden w-14 h-14 px-1 cursor-pointer"
              : " w-14 h-14 px-1 cursor-pointer"
          }
        />
        <GiNightSleep
          onClick={toggleIcon}
          className={
            dayMode? " hidden w-14 h-14" : " w-14 h-14 cursor-pointer"
          }
        />
      </div>
